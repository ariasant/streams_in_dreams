...
def test_snapshot_roundtrip(tmp_path):
    ...
    accel_a = np.random.rand(n_host, 3)
    mass_a = np.full(n_host, 0.02)
    flag_a = np.zeros(n_host, dtype=bool)
    writer.write(0, 0.0, pos_a, vel_a, mass_a, flag_a, accel_a)
    ...
    accel_b = np.vstack([accel_a, [[0.1, 0.0, 0.0]]])
    mass_b = np.append(mass_a, 0.2)
    flag_b = np.append(flag_a, True)
    writer.write(100, 2.0, pos_b, vel_b, mass_b, flag_b, accel_b)
