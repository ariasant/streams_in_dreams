...
def visualize_expansion(exp_builder, filename, parttype=None, label=None):
    ...
    for i,field in enumerate(["dens", "potl"]):
        surface_projection(basis=exp_builder.basis[1],
                        coefs=exp_builder.coefs[1],
                        exp_units=exp_builder.exp_units,
                        field=field, 
                        time=exp_builder.coefs[1].Times()[-1],
...
def main(snap_path,
    ...
    particles = MW_sim.__load_part_data__(snap=1099, 
                                          PartType=4)
    ...
    np.savez(os.path.join(output_path,"original_tracks.npz"),
    ...
    print(f"Reconstructed tracks for {len(bfe_tracks)} particles. Missing data for {missing_count} particles.")
    ...
    sim_orbits = calculate_orbital_parameters(sim_tracks)
    bfe_orbits = calculate_orbital_parameters(bfe_tracks)
    
    # Plot some of the orbits
    fig,axs = plt.subplots(10,3)
    for i,pid in enumerate(rng.choice(ids, size=10, replace=False)):
...
if __name__ == "__main__":
    ...
    output_path = "/mnt/home/asante/ceph/orbit_reconstruction/"
    n_particles = 10000
    z_range = [1,0]
